...
export async function fetchHistoricalData(
  ...
  const [quotesResponse, optionsResponse] = await Promise.all([
    fetchWithRetry(`${API_BASE_URL}/historicalQuotes?symbol=${stock}&days=${BATCH_SIZE}&skip=${loadedDays}`),
    fetchWithRetry(`${API_BASE_URL}/historicalOptions?symbol=${stock}&days=${BATCH_SIZE}&skip=${loadedDays}`)
  ]);
